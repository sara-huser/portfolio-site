  ...
  methods: {
    filterVideos: function(videoFilter, event) {
      ...
      if (videoFilter === 'view_all') {
        videos = this.$store.state.work_page.videos;
        EventBus.$emit('filter-clicked', videoFilter);
        var obj = {
          url: store.state.work_page.videos[0].url,
          title: store.state.work_page.videos[0].title,
          description: store.state.work_page.videos[0].description
        };
        EventBus.$emit('thumbnail-clicked', obj);
      } else {
        videos = this.$store.state.work_page.videos.filter(function(s) {
          if (s.category.indexOf(videoFilter) > -1) {
            EventBus.$emit('filter-clicked', s.category);
            var obj;
            for (var i = 0; i < store.state.work_page.videos.length; i++) {
              if (store.state.work_page.videos[i].category === s.category) {
                obj = {
                  url: store.state.work_page.videos[i].url,
                  title: store.state.work_page.videos[i].title,
                  description: store.state.work_page.videos[i].description
                };
              }
            }
            EventBus.$emit('thumbnail-clicked', obj);
