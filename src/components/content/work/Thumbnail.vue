<template>
  <div v-if="isLive" class="columns" @click="loadVideo()">
  ...
  <div v-else class="columns">
    <div class="video-thumbnail column is-half">
      <a class="thumbnail-link">
        <!-- <div v-bind:style="{ backgroundImage: 'url(' + imgUrl(image) + ')' }"></div> -->
        <!-- <img v-bind:src="require(`@/assets/img/thumbnails/${category}/${instructor}_${category}.png`)" /> -->
      </a>
    </div>
    <div class="video-info column is-half">
...
export default {
  ...
  store: store,
  props: ['isLive', 'url', 'title', 'description', 'category', 'instructor', 'playing', 'availableOn', 'image'],
  data() {
    return {
